...
6. ast-grep (optional) → run `sg --version 2>/dev/null` → ✅ available (AST-accurate structural search, 20+ languages) / ℹ️ not installed (optional — install with `npm i -g @ast-grep/cli` or `brew install ast-grep`)
```
