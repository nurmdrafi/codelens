...
1. **Check extraction data exists.** Run `Read` on `.codelens-review/extraction.json`.
   - If the file does not exist or is empty: STOP immediately. Write `.codelens-review/findings/architecture.json` with an error:
     ```json
     ...
     ```
   - If the file exists but contains a top-level `"error"` key: STOP immediately. Write `.codelens-review/findings/architecture.json` with the same error propagated:
     ```json
...
- If a context-mode tool call returns an error mid-run, write `.codelens-review/findings/architecture.json` with `"status": "partial_failure"` and the error details in `_methodology`. Do NOT silently fall back to Bash/Grep. The orchestrator will see the partial_failure status and skip merging incomplete findings.
